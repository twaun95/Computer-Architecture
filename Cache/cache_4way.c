...
for(way = 0; way < 4; way++){
    if(cache[way][index].valid != 1) ///////////cold miss
    {
	for(int i=0; i<16; i++)
	{
	cache[way][index].data[i] = Memory[(dest_addr/4) + i];
	printf("%d \n", cache[way][index].data[i]);
	}
    ...
    {
	if(cache[way][index].tag == tag)///hit
	{
	cache[way][index].sca = 1;
	return cache[way][index].data[offset/4];
	hit_count++;
	printf("hit\n");
	}

	else if(cache[way][index].tag != tag)/////////////miss
	{
	Miss_count++;
	}
    }
...
//////replacement
if(Miss_count == 4){//way 4개가 miss
	if(cache[oldest_way][index].dirty == 1)
	{
		for(int i=0; i<16; i++)//writeback
		{
		drain_addr = (cache[oldest_way][index].tag<<16) | (index<<6);
		Memory[drain_addr/4 + i] = cache[oldest_way][index].data[i];
		}
		cache[oldest_way][index].dirty = 0;
	}
	
	for(int i=0; i<16; i++)//fil
	{
	cache[oldest_way][index].data[i] = Memory[(dest_addr/4) + i];
	}

	cache[oldest_way][index].tag = tag;
	cache[oldest_way][index].valid = 1;
	cache[oldest_way][index].sca = 0;	
	miss_count++;
	Miss_count = 0;
	return cache[oldest_way][index].data[offset/4];
	oldest_way = (oldest_way + 1) % 4;
	}    
}
...
if(Miss_count == 4){//way 4개가 miss
		...
		cache[oldest_way][index].dirty = 0;
         ...
         cache[oldest_way][index].tag = tag;
         cache[oldest_way][index].valid = 1;
         cache[oldest_way][index].dirty = 1;
         cache[oldest_way][index].sca = 0;
	 cache[oldest_way][index].data[offset/4] = value;
         miss_count++;
	 Miss_count = 0;
         oldest_way = (oldest_way + 1) % 4;
}
